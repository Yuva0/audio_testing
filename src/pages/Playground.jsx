...
const Playground = () => {
  ...
  const [audioMap, setAudioMap] = useState({}); // { audio_1: base64, ... }
  const [timeMap, setTimeMap] = useState({}); // { time_1: ms, ... }
  const timeMapRef = useRef({});
  const [audioTimeMap, setAudioTimeMap] = useState({}); // { audio_time_1: ms, ... }
  ...
  const showNext = async (idx, list, audioMapArg, playgroundIdRef, timeMapArg = {}, audioTimeMapArg = {}) => {
    if (idx >= list.length) {
      setShowing(false);
      setUploading(true);
      // Send all times as a single PATCH
      if (playgroundIdRef.current) {
        await fetch('/api/playground', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: playgroundIdRef.current, ...timeMapRef.current, ...audioTimeMapArg }),
        });
        for (const [key, value] of Object.entries(audioMapArg)) {
          await fetch('/api/playground', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: playgroundIdRef.current, [key]: value }),
          });
        }
      }
      setUploading(false);
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          reader.onloadend = () => {
            ...
            // Only update state, do not call showNext here
            const newAudioMap = { ...audioMapArg, [`audio_${idx + 1}`]: audioBase64 };
            // If time not set, set to null
            ...
            // Now call showNext here, after audioBase64 is ready
            showNext(
              idx + 1,
              list,
              newAudioMap,
  ...
  const handleSubmit = async (e) => {
    ...
    const checkRes = await fetch(`/api/playground?name=${encodeURIComponent(name)}`);
    ...
    if (data.success) {
      ...
      setCurrentIdx(0);
      setShowing(true);
      // First, create the playground entry with name and media only
      const postRes = await fetch('/api/playground', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (postData.success && postData.id) {
        playgroundIdRef.current = postData.id;
        showNext(0, data.media, {}, playgroundIdRef);
      } else {
        setAudioError('Failed to create playground entry.');
        setShowing(false);
      }
    }
  };
